...
from . import metaimage, itklabels
...
def segment_volumes(volumes, model, *, side='left,right', tempdir=None):
    ...
    # save into temporaty directory, and segment
    segmented = {}
...
def _run_model(model, indir, outdir):
    ...
    for file in indir.glob('*'):
        print(f'\t{file}')
    labels = {1: 'DUMMY'}
    itklabels.write_labels(outdir / 'labels.txt', labels)
    vols = list(indir.glob('*_0000.mha'))
    for filename in vols:
        outname = filename.name.split('_0000')[0]
        vol, _ = metaimage.read(filename)
        roi = 1 * (vol > np.percentile(vol, 1))
        roi += (vol > np.percentile(vol, 50))
        metaimage.write((outdir / outname).with_suffix(".mha"), roi)
    input('(TMP) press any key to continue:')
